...
if [ -f "$ANALYSISDIR/$PROJ/pairs.txt" ]; then
    ...
    cat $ANALYSISDIR/$PROJ/pairs.txt | while read PAT TSAM NSAM; do
        echo "submitted:  sh $SCRIPTSDIR/mutect.sh $PAT $TSAM $NSAM $PROJ $BUILD $ANALYSISDIR. qsub_args: $PAT $PAT.log 8"
        q "sh $SCRIPTSDIR/mutect.sh $PAT $TSAM $NSAM $PROJ $BUILD $ANALYSISDIR" $PAT $PAT.log 8
    done
else 
